...
def add_vec_kernel(x_ptr, y_ptr, z_ptr, N0, N1, B0: tl.constexpr, B1: tl.constexpr):
    pid = tl.program_id(0)
    row_range = pid * B0 + tl.arange(0, B0)
    col_range = pid * B1 + tl.arange(0, B1)
    # 计算输出加载的索引
    grid = row_range[None, :] + B0 * col_range[:, None]
    x = tl.load(x_ptr + row_range)
    y = tl.load(y_ptr + col_range)
    # 自动广播计算
    z = x[None, :] + y[:, None]
    tl.store(z_ptr + grid, z)
...
def add_vec_block_kernel(
    ...
    row_offset = B0 * block_id_x + tl.arange(0, B0)
    col_offset = B1 * block_id_y + tl.arange(0, B1)
    z_off = row_offset[None, :] + N0 * col_offset[:, None]
    mask_x = row_offset < N0
    mask_y = col_offset < N1
    # 维度改变了，所以应该先广播再and
    mask_z = mask_x[None, :] & mask_y[:, None]

    x = tl.load(x_ptr + row_offset, mask=mask_x, other=0.0)
    y = tl.load(y_ptr + col_offset, mask=mask_y, other=0.0)
    z = x[None, :] + y[:, None]
    tl.store(z_ptr + z_off, z, mask=mask_z)
...
def mul_relu_block_back_kernel(
    ...
    x_offset = B0 * block_id_i + tl.arange(0, B0)
    y_offset = B1 * block_id_j + tl.arange(0, B1)
    xy_offset = x_offset[None, :] + N0 * y_offset[:, None]

    mask_y = y_offset < N1
    mask_x = x_offset < N0
    mask_xy = mask_x[None, :] & mask_y[:, None]

    x = tl.load(x_ptr + xy_offset, mask=mask_xy, other=0.0)
    y = tl.load(y_ptr + y_offset, mask=mask_y, other=0.0)
    dz = tl.load(dz_ptr + xy_offset, mask=mask_xy, other=0.0)
    xy = x * y[:, None]
    df = tl.where(xy > 0, 1.0, 0.0)
    out = df * y[:, None] * dz

    tl.store(dx_ptr + xy_offset, out, mask=mask_xy)
    return
...
def sum_kernel(x_ptr, z_ptr, N0, N1, T, B0: tl.constexpr, B1: tl.constexpr):
    ...
    pid = tl.program_id(0)
    # 纵向上的线程索引
    offset_x = tl.arange(0, B0) + B0 * pid
    # 纵向启动的线程索引需要小于纵向的与啊
    mask_x = offset_x < N0
    z = tl.zeros([B0], dtype=tl.float32)
    for idx in tl.range(0, T, B1):
        offset_y = tl.arange(0, B1) + idx
        offset_xy = T * offset_x[:, None] + offset_y[None, :]
        mask_y = offset_y < T
        mask_xy = mask_x[:, None] & mask_y[None, :]
        x = tl.load(x_ptr + offset_xy, mask=mask_xy)
        z += tl.sum(x, axis=1)

    tl.store(z_ptr + offset_x, z, mask_x)
...
def softmax_spec(x: Float32[4, 200]) -> Float32[4, 200]:
    ...
    x = x - x_max
    x_exp = x.exp()
    print(f"torch exp sum = {x_exp.sum(1, keepdim=True)}")
...
def softmax_kernel(x_ptr, z_ptr, N0, N1, T, B0: tl.constexpr, B1: tl.constexpr):
    ...
    seg_max_value = tl.full([B0], -float("inf"), dtype=tl.float32)
    max_value = tl.full([B0], -float("inf"), dtype=tl.float32)
    # 计算行的最大值
    for idx in tl.range(0, T, B1):
        offset_y = tl.arange(0, B1) + idx
        offset_xy = T * offset_x[:, None] + offset_y[None, :]
        ...
        x = tl.load(x_ptr + offset_xy, mask=mask_xy)
        # 计算当前段的最大值
        max_value = tl.maximum(tl.max(x, axis=1),seg_max_value)
        # 计算当前段的指数部分
        x_mean_exp = tl.exp2(log2_e * (x-max_value))
        # 保存当前最大值为
        frac = tl.exp2(log2_e*(seg_max_value-max_value))
        # 更新指数部分
        exp_sum = exp_sum*frac + tl.sum(x_mean_exp,axis=1)
        seg_max_value = max_value
        # print(f"local max = {seg_max_value} global max = {max_value}")
    ...
    for idx in tl.range(0, T, B1):
        ...
        mask_xy = mask_x[:, None] & mask_y[None, :]
        x = tl.load(x_ptr + offset_xy, mask=mask_xy)
        # 此时全局最大值和指数和的全局最大值已经更新了
        x_mean_exp = tl.exp2(log2_e * (x-max_value))
        z = x_mean_exp/exp_sum[None,:]
        tl.store(z_ptr + offset_xy, z, mask_xy)
...
def softmax_kernel_brute_force(
    ...
    for idx in tl.range(0, T, B1):
        ...
        mask_xy = mask_x[:, None] & mask_y[None, :]
        x = tl.load(x_ptr + offset_xy, mask=mask_xy)
        max_value = tl.maximum(max_value,tl.max(x, axis=1))
    ...
    for idx in tl.range(0, T, B1):
        ...
        x = tl.load(x_ptr + offset_xy, mask=mask_xy)
        # breakpoint()
        x = tl.exp2(log2_e * (x-max_value))
        # breakpoint()
        exp_sum += tl.sum(x, axis=1)


    # 计算最后的结果
    # TODO 为什么错误填充依然能得到正确的结果
    for idx in tl.range(0, T, B1):
        offset_y = tl.arange(0, B1) + idx
        offset_xy = T * offset_x[:, None] + offset_y[None, :]
        mask_y = offset_y < T
        ...
        exp_x = tl.exp2(log2_e * (x - max_value[:, None]))
        z = exp_x / exp_sum[:, None]
        tl.store(z_ptr + offset_xy, z, mask=mask_xy)
...
def flashatt_kernel(
    ...
    z = tl.zeros([B0], dtype=tl.float32)
    exp_sum = tl.zeros([B0], dtype=tl.float32)
    exp_sum_start = tl.zeros([B0], dtype=tl.float32)
    qk_max = tl.zeros([B0], dtype=tl.float32)
    # max_value = tl.full([B0], -float("inf"), dtype=tl.float32)
    local_max = tl.full([B0], -float("inf"), dtype=tl.float32)

    q = tl.load(q_ptr+offset_x,mask_x)
    for idx in tl.range(0, T, B1):
        offset_y = tl.arange(0, B1) + idx
        mask_y = offset_y < T
        mask_xy = mask_x[:, None] & mask_y[None, :]
        k = tl.load(k_ptr + offset_y, mask=mask_y)
        # breakpoint()
        qk = q[:,None]*k[None,:]+tl.where(mask_xy,0,-1.0e6)

        # 计算当前段的局部最大值
        qk_max = tl.maximum(qk_max,tl.max(qk, axis=1))
        # 根据局部最大值计算指数
        local_exp = tl.exp2(log2_e * (qk -qk_max[:, None]))
        # 计算补偿参数
        frac = tl.exp2(log2_e*(local_max - qk_max))
        # 更新补偿参数之后的结果
        exp_sum = frac*exp_sum_start+tl.sum(local_exp,axis=1)

        v = tl.load(v_ptr+offset_y,mask=mask_y,other=0.0)
        z = z *frac+tl.sum(local_exp*v[None,:],axis=1)
        local_max = qk_max
        exp_sum_start =  exp_sum
